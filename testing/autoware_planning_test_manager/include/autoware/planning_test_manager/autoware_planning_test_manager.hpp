...
public:
  ...
  // NodeT is templated so target_node can be an rclcpp::Node OR an
  // autoware::agnocast_wrapper::Node (which is not an rclcpp::Node). Existing rclcpp callers deduce
  // NodeT = rclcpp::Node and are unaffected.
  template <typename NodeT, typename InputT>
  void publishInput(
    const std::shared_ptr<NodeT> & target_node, const std::string & topic_name,
    const InputT & input, const int repeat_count = 3) const
  {
  ...
  // These are header-inline templates on NodeT (was non-template rclcpp::Node in the .cpp) so they
  // accept both rclcpp::Node and agnocast_wrapper::Node target nodes. Existing callers are unchanged.
  template <typename NodeT>
  void testWithNormalTrajectory(std::shared_ptr<NodeT> target_node, const std::string & topic_name)
  {
    publishInput(
      target_node, topic_name,
      autoware::test_utils::generateTrajectory<autoware_planning_msgs::msg::Trajectory>(10, 1.0), 5);
  }
  template <typename NodeT>

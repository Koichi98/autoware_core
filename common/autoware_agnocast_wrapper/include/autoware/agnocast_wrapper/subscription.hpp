...
class AgnocastSubscription : public Subscription<MessageT>
{
  ...
  /// Whichever of the two handles this subscription was constructed with. Both accessors below
  /// are on agnocast::SubscriptionBase, so neither cares which delivery mode is in use.
...
public:
  ...
  std::shared_ptr<const MessageT> take_data() override
  {
    ...
    return detail::to_std_shared_ptr(polling_handle().take(false));
  }

...
public:
  ...
  // parameter API (e.g. autoware::agnocast_wrapper::Node). It only reads parameters, so the body is
  // node-type agnostic. Defined inline because it is now a member template.

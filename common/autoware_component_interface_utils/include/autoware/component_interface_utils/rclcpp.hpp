...
/// Creates the endpoint wrappers for the interfaces a node provides and requires.
/// Templated on the node type, defaulting to rclcpp::Node.
...
public:
  /// Constructor. The passed node type D is a separate parameter from NodeT so that a derived node
  /// (`NodeAdaptor(this)`) does not deduce NodeT as that derived type; NodeT stays at its default
  /// unless spelled out (`NodeAdaptor<SomeNode>(this)`). D must be convertible to NodeT *.
  template <class D>
